...
def break_approvals_api():
    ...
    if request.method == 'GET':
        ...
        conn.close()
        return jsonify([{'approval_id': r[0], 'emp_id': r[1], 'emp_name': r[2], 'break_type': r[3], 'break_date': r[4].isoformat(), 'reason': r[5], 'status': r[6], 'approved_by': r[7], 'created_at': r[8].isoformat() + '+05:30' if r[8] else None} for r in rows]), 200
    data = request.get_json(silent=True) or {}
...
def get_break_types():
    ...
    for t in types:
        used = _scalar(
            ...
            [emp_id, t[0], shift_start_dt], conn)
        requires_approval = (t[0] == 'Lunch')
        approval_status = None
        if requires_approval:
            row = conn.execute(
                "SELECT status FROM break_approvals WHERE emp_id = ? AND break_type = ? AND break_date = ? ORDER BY created_at DESC LIMIT 1",
                [emp_id, t[0], shift_date]).fetchone()
            approval_status = row[0] if row else None
        result.append({'break_type': t[0], 'daily_limit_minutes': t[1], 'description': t[2],
                       'used_minutes': int(used or 0), 'requires_approval': requires_approval,
...
def get_login_hours():
    ...
    current_shift_date = _get_shift_start_dt(emp_id, conn).date()
    if date_str:
        try:
            target_date = datetime.strptime(date_str, '%Y-%m-%d').date()
        except ValueError:
            target_date = current_shift_date
    else:
        target_date = current_shift_date
    ...
    conn.close()
    return jsonify([{'login_time': s[0].strftime('%H:%M:%S') if s[0] else 'N/A', 'logout_time': s[1].strftime('%H:%M:%S') if s[1] else 'Active', 'total_hours': float(s[2]) if s[2] else 0, 'session_date': s[3].isoformat() if s[3] else None} for s in sessions]), 200

@attendance_bp.route('/api/user/shift-summary')
@login_required
def get_shift_summary():
    return jsonify(_compute_shift_summary(session['emp_id'], request.args.get('date', '')))


def _compute_shift_summary(emp_id, date_str=''):
    conn = get_db()
    current_shift_date = _get_shift_start_dt(emp_id, conn).date()
    if date_str:
        try:
    ...
    return {'date': target_date.isoformat(), 'shift_start': shift_start_dt.strftime('%H:%M'), 'shift_end': shift_end_dt.strftime('%H:%M'), 'first_login': first_login.strftime('%H:%M:%S') if first_login else None, 'last_logout': last_logout.strftime('%H:%M:%S') if last_logout else None, 'shift_hours': shift_hours, 'total_session_hours': total_session_hours, 'total_break_minutes': total_break_minutes, 'productive_hours': round(productive_hours, 2), 'efficiency': efficiency, 'session_count': session_count, 'break_count': len(breaks)}
...
def get_user_calendar():
    ...
    leave_map = {}
    for leave in leaves:
        ld_start, ld_end = leave[0], leave[1]
        if isinstance(ld_start, datetime):
            ld_start = ld_start.date()
        if isinstance(ld_end, datetime):
            ld_end = ld_end.date()
        current = ld_start
        while current <= ld_end:
            leave_map[current.isoformat()] = {'type': leave[2], 'status': leave[3]}
...
def get_dashboard_stats():
    return jsonify(_compute_dashboard_stats())
...
def _compute_dashboard_stats():
    ...
    for (eid,) in all_employees:
        sdt = _get_shift_start_dt(eid, conn).date()
        by_shift_date.setdefault(sdt, []).append(eid)
    logged_in_today = 0
    on_break = 0
    for sdate, eids in by_shift_date.items():
        if not eids:
            continue
        placeholders = ','.join('?' * len(eids))
        params = [sdate] + eids
        logged_in_today += _scalar(f"SELECT COUNT(DISTINCT emp_id) FROM user_sessions WHERE session_date = ? AND emp_id IN ({placeholders})", params, conn)
        on_break += _scalar(f"SELECT COUNT(DISTINCT emp_id) FROM breaks WHERE status = 'Active' AND break_date = ? AND emp_id IN ({placeholders})", params, conn)
    blocked = _scalar("SELECT COUNT(*) FROM users WHERE status = 'Blocked'", conn=conn)
    pending_leaves = _scalar("SELECT COUNT(*) FROM leave_requests WHERE status = 'Pending'", conn=conn)
    conn.close()
    return {'total_employees': total, 'logged_in_today': logged_in_today, 'on_break': on_break, 'blocked_users': blocked, 'pending_leaves': pending_leaves}
